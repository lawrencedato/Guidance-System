...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'activate') {
    ...
    if (!$check || $check->num_rows === 0) {
        echo json_encode([
            "success" => false,
            "message" => "No matching student found, or this account has been graduated/archived and cannot be activated."
        ]);
        exit;
    }

    // ================= STEP 2: Check if already activated (pre-check only) =================
    $already = $conn->query(
    ...
    try {
        // ── Concurrency Control: Lock the student row to prevent duplicate activation ──
        $lock = $conn->query("
            SELECT student_id FROM activated_students
            WHERE student_id = '$sid'
            FOR UPDATE
        ");

        // ── Re-check inside transaction (guards against race condition) ──
        if ($lock && $lock->num_rows > 0) {
            throw new Exception("This account has already been activated. Please proceed to login.");
        }

        // ── Insert the activated account ──
